...
var _initialiseProps = function _initialiseProps() {
  ...
  this.draw = function () {
    _this2._p.stroke(_this2._stroke);
    _this2._p.fill(_this2._fill);
    _this2._p.ellipse(_this2._x, _this2._y, _this2._radius);
    _this2.drawPetals();
    _this2.drawStem();
  };

  this.drawPetals = function () {
    var top = [_this2.getEllipsePoint(200), [-.16, -2], [0.5, -2.33], [0.6, -1.33], _this2.getEllipsePoint(320)];

    var topLeft = [_this2.getAnchorPoint(top, .4, 0, { repeatFirst: true }), [-0.83, -1.16], [-0.66, -2.16], _this2.getAnchorPoint(top, .2)];

    var topRight = [].concat(top, [_this2.getAnchorPoint(top, .5, 2, { repeatLast: true }), [1, -1.33], [1, -1.66], _this2.getAnchorPoint(top, .5, 1)]);

    var left = [[-1, -1.66], [-1.17, -1.33], [-1.33, 0], _this2.getEllipsePoint(100), [.33, 0], [-.5, -1], [-0.67, -1.67], [-1, -1.66]];

    var right = [_this2.getEllipsePoint(20), [1.33, -0.83], [1.33, -1.33], [1, -1.4], [0.5, -1], [0, -0.83], _this2.getEllipsePoint(130)];

    var rightTwo = [right[1], [1, .67], _this2.getEllipsePoint(100), [.1, .1], [1, -0.33], right[1]];

    _this2.drawCurve(topLeft);
    _this2.drawCurve(topRight);
    _this2.drawCurve(top);
    _this2.drawCurve(right);
    _this2.drawCurve(rightTwo);
    _this2.drawCurve(left);
  };

  this.getEllipsePoint = function (angle) {
    return [_this2._p.cos(_this2._p.radians(angle)), _this2._p.sin(_this2._p.radians(angle))];
  };
  ...
  this.getAnchorPoint = function (vertices, amount) {
    ...
    return [(_p4 = _this2._p).curvePoint.apply(_p4, _toConsumableArray(coords.map(function (_ref9) {
      var _ref10 = _slicedToArray(_ref9, 1),
          x = _ref10[0];

      return x;
    })).concat([amount])), (_p5 = _this2._p).curvePoint.apply(_p5, _toConsumableArray(coords.map(function (_ref11) {
  ...
  this.drawCurve = function (vertices) {
    ...
    var anchored = _this2.scale([vertices[0]].concat(_toConsumableArray(vertices), [[].concat(_toConsumableArray(vertices)).pop()]));
    _this2._p.beginShape();
    anchored.map(function (v) {
      var _p6;

      return (_p6 = _this2._p).curveVertex.apply(_p6, _toConsumableArray(v));
    });
    _this2._p.endShape();
  };
};
...
var _rose = __webpack_require__(0);
...
var Canvas = function Canvas() {
  ...
  this.sketch = function (p) {
    _this._p = p;
    _this._p.setup = _this.setup;
    _this._p.draw = _this.draw;
    _this._p.drawFns = new Set();
  };

  this.setup = function () {
    _this.canvas = _this._p.createCanvas(_this._p.displayWidth, _this._p.displayHeight);
    var smallDisplay = _this._p.displayWidth <= DIMENSION_THRESHOLD;
    var x = _this._p.displayWidth / 3,
        y = _this._p.displayHeight / 5;

    var opts = {
      x: x,
      y: y,
      radius: _this._p.displayWidth / (smallDisplay ? 10 : 30),
      stroke: '#fda732',
      fill: '#ffffff'
    };
    _this._rose = new _rose2.default(_this._p, opts);
    _this._p.drawFns.add(_this._rose.draw);
    var resume = new _resume2.default(_this._p, { x: x, y: y, smallDisplay: smallDisplay });
    resume.setup();
  };

  this.draw = function () {
    _this._p.clear();
    _this._p.drawFns.forEach(function (fn) {
      return fn();
    });
  };

  this.render = function () {
    return new p5(_this.sketch, _this._target);
  };
...
var Resume = function Resume(p) {
  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      x = _ref.x,
      y = _ref.y,
      smallDisplay = _ref.smallDisplay;
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.setup = function () {
    _this._button.position(_this._x * 1.25, _this._y * 2.5);
    _this._button.elt.style.visibility = 'visible';
    if (_this._smallDisplay) {
      _this._button.elt.style['font-size'] = '24px';
    }
    _this._button.elt.onkeypress = function (e) {
      if (e.keyCode === 31 || e.keyCode === 13) {
        _this.navigate();
      }
    };
    _this._button.mousePressed(function () {
  ...
  this.navigate = function () {
    if (_this.hasRose) {
      _this._p.drawFns.delete(_this.decorate);
    }
    _this.hasRose = false;
    if (_this._selected) {
      _this._selected.elt.style.visibility = 'hidden';
    }
    _this._selected = _this.sections.shift();
    if (_this._selected) {
      var _selected;

      var coords = _this._smallDisplay ? [_this._button.x, _this._button.y + _this._button.height * 2] : [_this._x * 1.75, _this._y];
      (_selected = _this._selected).position.apply(_selected, coords);
      _this._selected.elt.style.visibility = 'visible';
      _this.hasRose = _this._selected.class().split(' ').indexOf('decorate') !== -1;
    }
    if (_this.hasRose) {
      _this._p.drawFns.add(_this.decorate);
    }
    _this.sections.push(_this._selected);
  };

  this.decorate = function () {
    var _selected2 = _this._selected,
    ...
    var opts = {
      x: x + (_this._smallDisplay ? _this._button.width : width) / 2,
      y: y + height * 2.75,
      radius: width / 15
    };
    _this._rose = new _rose2.default(_this._p, opts);
